import cv2
import numpy as np
from tree import TupleTree, ImageTree
...
class ImageLinker():
	...
	def edgesToTree(self, firstelem, array, edge_table, data_table):
		tree = ImageTree(firstelem)
		self.__edge_rec(tree, array, edge_table, data_table)
		return tree

	def __edge_rec(self, tree, array, edge_table, data_table):
		...
		while any(line):
			...
			line = edge_table[index].tolist()
			self.__edge_rec(newTree, array, edge_table, data_table)